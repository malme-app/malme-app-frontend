...
import { MSG_INVITE_FAILED, MSG_INVITE_SUCCESS, MSG_SERVER_ERROR, MSG_UPDATE_FAILED, MSG_UPDATE_SUCCESS } from 'src/app/helper/notificationMessages';
...
export class InviteComponent implements OnInit {
  ...
  updateRole(email: string) {
    this.http
      .patch(
        `${environment.apiBaseUrl}/user/role`,
        { email }
      )
      .subscribe({
        next: (_data) => {
      ...
      .subscribe({
        next: (data: any) => {
          ...
          data.userList.forEach((element: any) => {
            dataSource.push({
              email: element.email,
              role: element.roles.includes('SuperAdmin') ? '最高管理者' : (element.roles.includes('Admin') ? '管理者' : 'メンバー'),
              status: '',
              action: !element.roles.includes('SuperAdmin') && element.roles.includes('Admin') ? '管理権限を削除する' : (!element.roles.includes('Admin') ? '管理権限を委託する' : '')
            })
          });
